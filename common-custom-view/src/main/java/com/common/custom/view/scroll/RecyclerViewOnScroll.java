...
package com.common.custom.view.scroll;
...
public class RecyclerViewOnScroll extends RecyclerView.OnScrollListener {
	private CmRecyclerView mPullLoadMoreRecyclerView;

	public RecyclerViewOnScroll(CmRecyclerView pullLoadMoreRecyclerView) {
	...
	public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
		...
		if (layoutManager instanceof GridLayoutManager) {
			...
			// since may lead to the final item has more than one StaggeredGridLayoutManager
			// the particularity of the so here that is an array
			// this array into an array of position and then take the maximum value that is
			// the last show the position value
		...
		if (mPullLoadMoreRecyclerView.isLoadMoreEnabled() && !mPullLoadMoreRecyclerView.isRefreshing()
				&& !mPullLoadMoreRecyclerView.isNoMore() && (lastCompletelyVisibleItem == totalItemCount - 1)
				&& !mPullLoadMoreRecyclerView.isLoadingData() && (dx > 0 || dy > 0)) {
