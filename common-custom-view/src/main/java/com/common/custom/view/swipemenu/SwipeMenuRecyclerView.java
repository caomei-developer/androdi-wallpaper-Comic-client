...
package com.common.custom.view.swipemenu;
			...
			case MotionEvent.ACTION_UP:
				...
				if (isOutDuration || isOutX || isOutY) {
					...
					if (upView instanceof SwipeMenuLayout) {
						...
						// intercept the up event when touched on the contentView of the opened
						// SwipeMenuLayout
						if (!(x >= menuView.getLeft() + translationX && x <= menuView.getRight() + translationX
								&& y >= menuView.getTop() + translationY && y <= menuView.getBottom() + translationY)
								&& isCloseOnUpEvent) {
							return true;
						}
					}
