package com.common.custom.view.adapter;
...
public class WrapperRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
	 ...
	 * 设置自定义的FooterView
	 */
	public WrapperRecyclerAdapter setLoadMoreFooter(CmLoadMoreFooter loadMoreFooter) {
		...
		if (realPosition >= 0 && realPosition < adapter.getItemCount()) {
			if (isReservedItemType(adapter.getItemViewType(realPosition))) {
				throw new IllegalStateException(
						"ZRecyclerView require itemViewType in adapter should be less than 10000 ");
			}
	...
	public boolean isFooterView(int position) {
		return mFooterLayout != null && ((isLoadMoreEnabled && position == getItemCount() - 2)
				|| (!isLoadMoreEnabled && position == getItemCount() - 1));
	}
	...
	public void onViewAttachedToWindow(RecyclerView.ViewHolder holder) {
		...
		if (lp != null && lp instanceof StaggeredGridLayoutManager.LayoutParams) {
			if (isReservedItemType(getItemViewType(position))
					|| (adapter instanceof BaseRecyclerAdapter && ((BaseRecyclerAdapter) adapter)
